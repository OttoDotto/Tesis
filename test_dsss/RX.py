...
CANTIDAD_SIMBOLOS_TRAMA = (
    len(PREAMBULO)
    + CANTIDAD_CHIPS_DATOS
    + len(GUARDA)
)

# Tamaño de una trama completa, en muestras. Se usa para saber
# cuánta "cola" hay que arrastrar de una lectura del SDR a la
# siguiente, de forma que ningún paquete que empiece cerca del
# borde de un bloque quede cortado a la mitad.
MUESTRAS_TRAMA = (
    CANTIDAD_SIMBOLOS_TRAMA
    * MUESTRAS_POR_BIT
)
...
def analizar_trama(
    ...
    cantidad_simbolos = CANTIDAD_SIMBOLOS_TRAMA

    cantidad_muestras = MUESTRAS_TRAMA
...
print(
    f"Muestras/trama: {MUESTRAS_TRAMA}"
)
...
# Cola de muestras arrastrada entre lecturas consecutivas del
# SDR, para que un paquete que empiece cerca del final de un
# bloque no quede cortado a la mitad.
buffer_acumulado = np.zeros(
    0,
    dtype=np.complex64
)
...
try:
    ...
    while True:
        ...
        nuevas_muestras = buffer_rx[
            :cantidad
        ].copy()

        # Pegamos la cola de la lectura anterior adelante de
        # las muestras nuevas.
        ...
        # Por defecto, si no pasa nada especial, la próxima
        # cola es simplemente la última porción del bloque
        # actual (tamaño de una trama completa).
        buffer_acumulado = muestras[
            -MUESTRAS_TRAMA:
        ] if len(muestras) > MUESTRAS_TRAMA else muestras
        ...
        if resultado_trama is None:
            ...
            # La trama empieza dentro de este bloque pero se
            # extiende más allá de lo leído. En vez de
            # descartarla, arrastramos desde "inicio" para
            # completarla con la próxima lectura del SDR.
            buffer_acumulado = muestras[
                inicio:
            ]
            ...
            print(
                ...
                "Se completará con la próxima lectura."
            )

            continue

        # Trama procesada por completo: avanzamos la cola más
        # allá de esta trama para no volver a detectarla.
        buffer_acumulado = muestras[
            inicio + MUESTRAS_TRAMA:
        ]
